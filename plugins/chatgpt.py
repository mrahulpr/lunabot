...
async def chatgpt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await send_error_to_support(f"`chatgpt_command`\n\n{traceback.format_exc()}")
...
async def chatgpt_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if data == "chatgpt_toggle_user":
            ...
            await query.answer("✅ Toggled group setting")
        ...
        await send_error_to_support(f"`chatgpt_callback`\n\n{traceback.format_exc()}")
...
async def chatgpt_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await send_error_to_support(f"`chatgpt_reply`\n\n{traceback.format_exc()}")
...
async def test():
    ...
    await db.command("ping")                    f"{'✅ ' if user.id in new_enabled_users else ''}Enable for me only",
                    callback_data="chatgpt_toggle_user"
                ),
                InlineKeyboardButton(
                    f"{'✅ ' if new_group_enabled else ''}Enable in group for all",
...
async def test():
    assert OPENAI_API_KEY, "ChatGPT API key not set"
    assert db is not None, "MongoDB not connected"
    await db.command("ping")        await query.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(buttons))
    except Exception:
